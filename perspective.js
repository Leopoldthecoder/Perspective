...
perspective.hover = function(isHoming) {
  ...
  Array.from(divs).forEach(div => {
    if (div.getAttribute('data-perspective') === 'hover') {
      containers.push(div)
    }
  })
  for (i = 0; i < containers.length; i++) {
    (function(i) {
      ...
      function setTranslate(ele, x, y) {
        const prefix = ['webkitTransform', 'msTransform', 'transform']
        for (let j = 0; j < prefix.length; j++) {
          ele.style[prefix[j]] = 'translate(' + x + 'px,' + y + 'px)'
        }
      }
      function relocate(ele, x, y, v) {
        const offsetX = Math.floor(v * (0.5 * document.body.clientWidth - x))
        const offsetY = Math.floor(v * (0.5 * document.body.clientHeight - y))
